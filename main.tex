...
\begin{abstract}
    A key aspect of photon interaction with matter involves Compton scattering, in which an incident photon elastically scatters off of a discharged electron, the energies of which can be modeled kinematically.
    ...
    We collimated the source with lead bricks and configured the organic and inorganic detectors to measure Compton scattering in the organic detector, with Compton electron energies measured by the organic detector, and deflected photons striking the inorganic detector (positioned at specific angles of 0, 30, 60, and 90 degrees), with a fast digitizer to record only simultaneous responses from both detectors (within a window of 96 ns).\\
...
While Compton scattering can take place at any incident photon energy, it is most significant in the intermediate energies between where photoelectric effect and pair production dominate, and especially for lower-z material, where neither dominates, as seen in \figref{fig:intr0_effects}.\\
...
Compton scattering is of particular interest due to its kinetic model, as shown in \figref{fig:intr1_kine}, in which the model of an elastic collision, accounting for relativistic behavior, can be used to predict energies based on deflection angle $\theta$.
...
We arranged the detectors, source, and collimators in a similar fashion to the diagram of \figref{fig:Lab5_setupnot}, as can be seen in \figref{fig:mat0_setup}.

\defplot{Lab5_setupnot}{A similar experimental setup to the one used in this laboratory. Unlike this setup, measurements were performed for inorganic detector placement at angles of 0, 30, 60, and 90 degrees. Sourced from lab manual.\cite{npre_lab_5_manual_2025}}
...
\begin{figure}
    \centering\includegraphics[width=0.6\linewidth]{fig/mat1_digi.jpg}
...
Placing the \CsSrc source in front of the organic detector, we measured the spectrum of the organic scintillator's response, to use for energy calibration.\\

To record such a spectrum, we open the computer (connected to the digitizer by USB cable) and start the program Compass, selecting 'Scan' to connect to the digitizer board.
We clicked `File/New Project' and created a new folder to save acquisition settings and data files, selecting a new folder for each spectrum measurement.
This prompted a window with acquisition settings.
In the `Acquisition' tab, we chose a memorable `Run ID' to name the folder to store files in, and checked the `auto-increment' option to avoid overwriting files.
We set the desired time, in seconds, in the `Timed run (s)' tab, checked `Save all spectra on stop acquisition', and checked `Energy' and `$\Delta$T' spectra options.\\

In the `Settings' tab, we selected `Input' and `Discrimination' settings shown in \figref{fig:set_input} and \figref{fig:set_discrim}; selected under `QDC' settings a gate of 800 ns, short gate of 400 ns, and pre-gate of 48 ns; and selected under `Onboard coincidences' settings a coincidence window of 96 ns.
...
Placing the \CsSrc source directly in front of the inorganic detector, we measured the spectrum of the inorganic scintillator's response for calibration.\\
...
Ensuring both detectors were properly biased and connected to the digitizer, we recorded the coincidental response spectrum of both detectors for 20 minutes in the 0\degr configuration.
